...
```bash
ssh web@example.com "grep -i '\"https://www.google.com/' /srv/www/example.com/logs/access.log \
...
```bash
ssh web@example.com "tail -n 50000 /srv/www/example.com/logs/access.log \
...
```bash
ssh web@example.com "grep '/your-post-slug/' /srv/www/example.com/logs/access.log \
...
- `/author/username/` appearing in organic referrals is a signal to add it to the noindex queue
