...
func TestCommandsInDisplayPreservesCategoryForJSON(t *testing.T) {
	...
	if len(scriptsRaw) != 45 {
		t.Errorf("CommandsIn(scripts) = %d entries, want 45 (directory-based grouping must be unaffected by the display split; +2 for ttfb-test/remote-ttfb-ua added in 3.27.0, +3 for svg-to-jpg/svg-to-png/traffic-by-country added in 4.1.0, +1 for wp-db-backup added in 5.0.0, +1 for updraft-to-valet added in 5.9.0, +1 for scripts/release/sync-extracted added in 5.11.0, +2 for publish-post/verify-post added in 5.16.0, +1 for ssl-check added in 5.21.0)", len(scriptsRaw))
	}
